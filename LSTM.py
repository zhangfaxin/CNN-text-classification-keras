...
model.add(Embedding(input_dim=len(embeddings_matrix), output_dim=embedding_dim,weights=[embeddings_matrix],input_length=sequence_length,trainable=False))
model.add(LSTM(128,dropout=0.2))
model.add(Dense(units=2, activation='softmax'))
model.summary()

checkpoint = ModelCheckpoint('./lstm/lstm_word_pianpang_weights.{epoch:03d}-{val_acc:.4f}.hdf5', monitor='val_acc', verbose=1, save_best_only=True, mode='auto')
