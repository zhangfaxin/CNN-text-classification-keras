from keras.layers import Input, Dense, Embedding, Conv2D, MaxPool2D, LSTM, GlobalAveragePooling1D
...
checkpoint = ModelCheckpoint('./fastText/fastText_word_pianpang_weights.{epoch:03d}-{val_acc:.4f}.hdf5',
                             monitor='val_acc', verbose=1, save_best_only=True, mode='auto')
adam = Adam(lr=1e-3, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.0)

model.compile(optimizer=adam, loss='binary_crossentropy', metrics=['accuracy'])
# TensorBoard
tbCallBack = TensorBoard(log_dir='./fastText/', update_freq='batch')
# EalryStopping
