...
lr = LogisticRegressionCV(multi_class="ovr",fit_intercept=True,Cs=np.logspace(-2,2,20),cv=2,penalty="l2",solver="sag",tol=0.01)
...
evaluate = pd.DataFrame({'review': x_raw, 'trueL': y_eval, 'preL': pre})
evaluate.to_csv('./data/cnn_combine_word_word_evaluate-pianpang.csv', header=True, index=False)
